...
namespace ArcFrame.Core.Math
{
    ...
    {
        double[,] A, Ehalf, Efull;
        double[] Tmid, k;
        double srow;

        private void EnsureCapacity(int N)
        {
            if (A != null && N <= A.GetLength(0)) return;

            A = new double[N, N];
            Ehalf = new double[N, N];
            Efull = new double[N, N];
            Tmid = new double[N];
            k = new double[N];
        }
        /// <summary>
        ...
        {            
            int N = R.GetLength(0);
            EnsureCapacity(N);
            k = kappa(s + 0.5 * h);                  // midpoint sample (size N-1)

            // Build skew A at the midpoint
            /*A = (frame == FrameModel.Frenet)
                  ? ONFrame.BuildFrenetSkew(k)                   // uses FS invariants
                  : ONFrame.BuildBishopSkew(k);                  // uses Bishop components
            */

            if (frame == FrameModel.Frenet) ONFrame.BuildFrenetSkew(ref A, k);
            else ONFrame.BuildBishopSkew(ref A, k);

            // Exponentials on SO(N)
            MatrixExp.ExpSkew(A, 0.5 * h, ref Ehalf);
            MatrixExp.ExpSkew(A, h, ref Efull);
            //Ehalf = MatrixExp.ExpSkew(A, 0.5 * h);
            //Efull = MatrixExp.ExpSkew(A, h);
